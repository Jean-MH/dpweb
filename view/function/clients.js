...
async function view_clients() {
  try {
    ...
    let json = await respuesta.json();
    let contenedor = document.getElementById("content_clients");
...
async function view_proveedores() {
  try {
    let respuesta = await fetch(
      base_url + "control/ClientsController.php?tipo=ver_proveedor",
      {
        method: "GET", 
        mode: "cors",
        cache: "no-cache",
...
async function actualizarProveedor() {
  const datos = new FormData(document.querySelector("#frm_edit_proveedor"));
  let respuesta = await fetch(
    base_url + "control/ClientsController.php?tipo=actualizar",
    {
      method: "POST",
